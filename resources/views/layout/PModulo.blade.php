...
<body>
    <div class="cont-principal">
        ...
        <main>
            <div class="cont-nav-modulos">
                <ul class="nav-modulos">
                    @php
                        $num = 1;
                        $x = true;
                    ...
                    @if (count($buscarTest)>0 && ($inscripcionAlumno[0]->identificacion == Auth::user()->identificacion || Auth::user()->admin == 1 || $buscarCurso[0]->identificacion == Auth::user()->identificacion))
                        <li><a href="{{route('test',['curso'=> $buscarCurso[0],'modulo'=> $modulos[0],'test' => $buscarTest[0]])}}" class="{{request()->routeIs('test')?'modulo-seleccionado': ''}}" >Test</a></li>
                    @endif
                </ul>
            </div>
